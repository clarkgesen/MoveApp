...
$(document).ready(() => {
  $.get('/api/activities').then((data) => {
    const activitydate = new Date(data[0].createdAt);
    ...
    const chestcount = 0;
    for (i = 0; i < data.length; ++i) {
      if (today.getMonth() === activitydate.getMonth() && data[i].bodypart === 'chest') { armscount++; }
    }
    const chestpercent = ((chestcount / 5) * 100);

    const cardiocount = 0;
    for (i = 0; i < data.length; ++i) {
      if (today.getMonth() === activitydate.getMonth() && data[i].bodypart === 'cardio') { armscount++; }
    }
    ...
    Highcharts.chart('activeContainer', {
